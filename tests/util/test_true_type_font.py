...
class TestTrueTypeFont(TestCase):
    ...
    def test_measure_missing_chars_equal_width(self):
        ...
        width_a = self.font.measure_text(char_a, self.font_size)
        width_b = self.font.measure_text(char_b, self.font_size)
        assert width_a == width_b

    def test_composability(self):
        missing_char = 'Ꮤ'
        text = 'Hi there, missing char is: '
        assert self.font.measure_text(text + missing_char, self.font_size) == (
            (
                self.font.measure_text(text, self.font_size) +
                self.font.measure_text(missing_char, self.font_size)
            )
        )
    ...
    def test_font_cache(self):
        ...
        with mock.patch('pdf_annotate.util.true_type_font.TrueTypeFont') as mock_get:
            get_true_type_font(HELVETICA_PATH, 'newfont')
